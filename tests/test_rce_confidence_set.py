...
def test_rce_zero_variance_coordinate_can_be_an_exact_equality() -> None:
    confidence = RCEConfidenceSet.from_diagonal_variances(
        np.asarray([1.0, 0.0, 4.0], dtype=np.float64),
        enforce_zero_variance_equalities=True,
    )
    feasible = confidence.evaluate(np.asarray([0.2, 0.0, -0.5]))
    infeasible = confidence.evaluate(np.asarray([0.2, 1.0e-6, -0.5]))

    assert feasible.inside is True
    assert infeasible.inside is False
    assert infeasible.max_exact_abs_residual == 1.0e-6
    assert np.array_equal(confidence.exact_mask, np.asarray([False, True, False]))
    assert np.array_equal(confidence.tube_mask, np.asarray([True, True, True]))
    assert confidence.diagnostics()["exact_equality_dimension"] == 1
