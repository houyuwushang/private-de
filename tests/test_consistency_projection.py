...
@pytest.mark.parametrize("projector", [project_query_space_lsq, project_query_space_feasible_lsq])
def test_query_space_projection_is_contractive_to_a_feasible_truth_in_weighted_norm(projector) -> None:
    builder = QueryBuilder(max_terms=1)
    builder.add([(0, OP_EQ, 0, 0, 0)], "a=0", "oneway:0", "oneway")
    builder.add([(0, OP_EQ, 1, 1, 1)], "a=1", "oneway:0", "oneway")
    qcat = builder.build()
    truth = np.asarray([7.0, 3.0], dtype=np.float64)
    noisy = np.asarray([-10.0, 25.0], dtype=np.float32)
    variances = np.asarray([1.0, 9.0], dtype=np.float32)

    result = projector(noisy, qcat, np.asarray([2], dtype=np.int32), total=10, variances=variances)

    before = float(np.sum((noisy.astype(np.float64) - truth) ** 2 / variances))
    after = float(np.sum((result.projected.astype(np.float64) - truth) ** 2 / variances))
...
def test_local_table_feasible_jax_caps_dense_hessian_before_allocation() -> None:
    builder = QueryBuilder(max_terms=1)
    builder.add([(0, OP_EQ, 0, 0, 0)], "a=0", "oneway:0", "oneway")
    qcat = builder.build()

    with pytest.raises(ValueError, match="dense QP Hessian"):
        project_local_table_feasible_jax(
            np.asarray([1.0], dtype=np.float32),
            qcat,
            np.asarray([100], dtype=np.int32),
            total=10,
            variances=np.ones(1, dtype=np.float32),
            max_scope_cells=100,
            max_dense_constraint_cells=1_000,
        )
