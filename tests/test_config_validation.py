...
def test_rce_v1_reference_path_is_allowed() -> None:
    validate_config(_rce_config())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda cfg: cfg["qdte"].update(confidence_stop={"enabled": True}),
            "disable confidence_stop",
        ),
        (
            lambda cfg: cfg["qdte"]["rce"].update(alpha_l2=0.05),
            "alpha_l2=alpha_linf=0.025",
        ),
        (
            lambda cfg: cfg["qdte"]["rce"].update(product_prior_smoothing=0.5),
            "product_prior_smoothing=1.0",
        ),
        (
            lambda cfg: cfg["qdte"].update(entropy={"enabled": True}),
            "mutually exclusive",
        ),
    ],
)
def test_rce_v1_fails_closed(mutation, message: str) -> None:
    cfg = _rce_config()
    mutation(cfg)
