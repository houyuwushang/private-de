...
def test_dp_measurement_calibrates_noise_to_workload_group_vectors() -> None:
    ...
    m = measure_real_dataset(
        ...
        np.random.default_rng(0),
        batch_size=4,
        cardinalities=np.asarray([3], dtype=np.int32),
    )
...
def test_dp_measurement_requires_public_cardinalities() -> None:
    ...
    cfg = {"privacy": {"mode": "dp", "rho_total": 1.0}, "projection": {}}
...
def test_static_measurement_rejects_overlapping_or_missing_query_groups() -> None:
    ...
    qcat = builder.build()
    groups = [
        WorkloadGroup("g0", "oneway", np.asarray([0], dtype=np.int32), 1.0, False),
        WorkloadGroup("g1", "oneway", np.asarray([0], dtype=np.int32), 1.0, False),
    ]
    X = np.asarray([[0], [1]], dtype=np.int32)
    cfg = {"privacy": {"mode": "dp", "rho_total": 1.0}, "projection": {}}
