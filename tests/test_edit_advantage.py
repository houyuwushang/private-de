...
from qdte.evolution.scoring import (
    edit_advantage_from_delta,
    l1_advantage_from_delta,
...
def test_l1_advantage_identity() -> None:
    rng = np.random.default_rng(1)
    residual = rng.normal(size=20).astype(np.float32)
    weights = rng.uniform(0.1, 2.0, size=20).astype(np.float32)
    delta = rng.integers(-1, 2, size=(5, 20), dtype=np.int8)
    cost = np.zeros(5, dtype=np.float32)
    adv = l1_advantage_from_delta(residual, weights, delta, cost, lambda_cost=0.0)
    for i in range(5):
        objective_before = np.sum(np.abs(residual) * weights)
        objective_after = np.sum(np.abs(residual - delta[i].astype(np.float32)) * weights)
        assert np.allclose(objective_after - objective_before, -adv[i], atol=1e-5)
...
def test_pmap_and_jit_candidate_scores_match() -> None:
    ...
    context_scores = score_candidates(
        candidates,
        residual,
        inv_var,
        qcat,
        ...
        chunk_size=4,
        use_pmap=False,
        context=prepare_score_context(qcat),
    )
    assert np.allclose(context_scores, jit_scores, atol=1e-5)
...
def test_l1_candidate_scores_match_delta_identity() -> None:
    builder = QueryBuilder(max_terms=1)
    builder.add([(0, OP_EQ, 1, 1, 1)], "a=1", "oneway:0", "oneway")
    builder.add([(1, OP_LE, 2, 0, 2)], "b<=2", "prefix:1", "prefix")
    qcat = builder.build()
    ...
    residual = np.asarray([5.0, -3.0], dtype=np.float32)
    weights = np.asarray([0.5, 2.0], dtype=np.float32)
    scores = score_candidates_l1(candidates, residual, weights, qcat, lambda_cost=0.1, chunk_size=4, use_pmap=False)
    deltas = np.asarray(
        [
            [1, 0],
            [0, 1],
    ...
    expected = l1_advantage_from_delta(
        residual,
        weights,
        deltas,
        np.ones(len(old_rows), dtype=np.float32),
        lambda_cost=0.1,
    )
