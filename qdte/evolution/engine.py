...
def run_qdte(config: dict[str, Any]) -> dict[str, Any]:
    ...
    rce_controller: RCEQDTEController | None = None
    ...
    if rce_enabled:
        if precision_operator_name != "orthogonal_interaction" or not isinstance(
            ...
            raise ValueError("RCE-v1 requires unshrunk exact orthogonal interaction precision")
    ...
    for iteration in range(1, effective_max_iters + 1):
        ...
        elif rce_controller is not None:
            if fused_advantages is not None:
                raise RuntimeError("RCE-v1 does not support fused GPU candidate scoring")
            if not isinstance(optimization_precision, OrthogonalInteractionPrecision):
                raise RuntimeError("RCE-v1 lost its orthogonal precision operator")
        ...
        if rce_controller is not None:
            ...
            def rce_prefix_evaluator(
                selected_indices: np.ndarray,
                delta_prefix: np.ndarray,
                cost_prefix: np.ndarray,
            ) -> np.ndarray:
                prefix_indices = np.asarray(selected_indices, dtype=np.int32)
                return rce_controller.prefix_advantages(
                    query_residual=state.residual,
                    query_delta_prefix=np.asarray(delta_prefix, dtype=np.float64),
    ...
    if rce_controller is not None:
        ...
        log(
            ...
            f"ellipsoid_ratio={rce_snapshot.confidence.ellipsoid_ratio:.6g}, "
            f"tube_ratio={rce_snapshot.confidence.tube_ratio:.6g}, "
            f"slack={rce_snapshot.confidence.slack:.6g}"
        )
