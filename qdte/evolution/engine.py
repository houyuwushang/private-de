...
from qdte.evolution.coarse_scoring import prepare_coarsened_precision_score_context
...
from qdte.measurement.cdwf_transcript import CDWFSequentialTranscript
...
from qdte.rce.coarse_controller import CoarsenedRCEQDTEController
from qdte.rce.sequential_forest_prior import (
    build_sequential_confidence_forest_prior,
)
from qdte.rce.streamwise_controller import StreamwiseRCEQDTEController
from qdte.rce.c0 import C0_PROTOCOL, c0_diagnostic_measurements, c0_support_coarsening
from qdte.rce.causal_diagnostics import (
...
def run_qdte(config: dict[str, Any]) -> dict[str, Any]:
    ...
    rce_cfg = qdte_cfg.get("rce", {}) or {}
    streamwise_cdwf_cfg = rce_cfg.get("streamwise_cdwf", {}) or {}
    streamwise_cdwf_enabled = bool(streamwise_cdwf_cfg.get("enabled", False))
    rce_prefix_backend = str(
        rce_cfg.get("prefix_backend", "feature_cpu_exact")
    ).lower()
    c0_cfg = rce_cfg.get("c0_diagnostic", {}) or {}
    c0_enabled = bool(c0_cfg.get("enabled", False))
    precision_homotopy_cfg = (
        rce_cfg.get("precision_homotopy_diagnostic", {}) or {}
    )
    precision_homotopy_enabled = bool(
        precision_homotopy_cfg.get("enabled", False)
    )
    measurement_cfg = config.get("measurement", {})
    ...
    if c0_enabled:
        if X_real is None:
            raise RuntimeError("Offline C0 diagnostic requires the private encoded table")
        if measurements.strategy_transcript is None:
            raise ValueError("Offline C0 diagnostic requires an interaction transcript")
        released_c0_transcript = HierarchicalInteractionTranscript.from_public_dict(
            measurements.strategy_transcript
        )
        c0_support = c0_support_coarsening(
            released_c0_transcript,
            X_real,
            str(c0_cfg.get("support_mode", "released")),
        )
        measurements, _ = c0_diagnostic_measurements(
            measurements,
            qcat,
            workload_groups,
            X_real,
            str(c0_cfg.get("interaction_center", "dp")),
        )
        log(
            "Enabled offline C0 diagnostic: "
            f"support={c0_cfg.get('support_mode', 'released')}, "
            f"interaction_center={c0_cfg.get('interaction_center', 'dp')}, "
            f"support_hash={c0_support.to_dict()['sha256']}"
        )
    elif precision_homotopy_enabled:
        if X_real is None:
            raise RuntimeError(
                "Offline precision homotopy requires the private encoded table"
            )
        if measurements.strategy_transcript is None:
            raise ValueError(
                "Offline precision homotopy requires an interaction transcript"
            )
        released_homotopy_transcript = (
            HierarchicalInteractionTranscript.from_public_dict(
                measurements.strategy_transcript
            )
        )
        c0_support = c0_support_coarsening(
        ...
        measurements, _ = precision_homotopy_measurements(
            measurements,
            qcat,
            workload_groups,
            X_real,
            precision_homotopy_cfg.get("gamma", 1.0),
        )
        log(
            "Enabled offline interaction precision homotopy: "
            f"gamma={precision_homotopy_cfg.get('gamma', 1.0)}, "
            f"support_hash={c0_support.to_dict()['sha256']}"
        )
    ...
    sequential_cdwf_transcript: CDWFSequentialTranscript | None = None
    coarse_interaction_transcript: CoarseInteractionTranscript | None = None
    if precision_operator_name in {
        ...
        if streamwise_cdwf_enabled:
            if measurements.sequential_transcript is None:
                raise ValueError(
                    "C3 streamwise RCE requires its released sequential transcript"
                )
            sequential_cdwf_transcript = CDWFSequentialTranscript.from_public_dict(
                measurements.sequential_transcript
            )
            expected_combined = sequential_cdwf_transcript.combined_transcript()
            if expected_combined.to_public_dict() != interaction_transcript.to_public_dict():
                raise ValueError(
                    "C3 sequential streams do not reconstruct the released combined transcript"
                )
    ...
    elif precision_operator_name == "coarsened_interaction_full_covariance":
        if measurements.strategy_transcript is None:
            raise ValueError(
                "coarsened interaction precision requires its released strategy transcript"
            )
        coarse_interaction_transcript = CoarseInteractionTranscript.from_public_dict(
            measurements.strategy_transcript
        )
        ...
        if (
            coarse_interaction_transcript.support.original_cardinalities
            != expected_cardinalities
        ):
            raise ValueError(
                "Coarsened interaction transcript cardinalities do not match the public schema"
            )
        if coarse_interaction_transcript.public_total != mechanism_num_rows:
            raise ValueError(
                "Coarsened interaction transcript public_total does not match the declared row count"
            )
        if not np.isclose(
            coarse_interaction_transcript.rho_spent,
            measurements.rho_spent,
            rtol=1.0e-10,
            atol=1.0e-12,
        ):
            raise ValueError(
                "Coarsened interaction transcript rho ledger does not match the measurement artifact"
            )
        projection_diagnostics = measurements.projection_diagnostics or {}
        if projection_diagnostics.get("required_precision_operator") != (
            "released_support_coarsened_interaction_full_covariance"
        ):
            raise ValueError(
                "Coarsened interaction artifact does not require exact full-covariance precision"
            )
        optimization_precision = CoarsenedInteractionPrecision(
            qcat,
            workload_groups,
            coarse_interaction_transcript,
        )
        reconstruction_residual = optimization_precision.target_reconstruction_residual(
            measurements.target_projected
        )
        float32_tolerance = (
            8.0
        ...
        if reconstruction_residual > float32_tolerance:
            raise ValueError(
                "Coarsened interaction target does not reconstruct its released "
                "coefficient center; "
                f"residual={reconstruction_residual:.6g}, "
                f"tolerance={float32_tolerance:.6g}"
            )
        precision_extra_diagnostics = {
            "measurement_mode": coarse_interaction_transcript.mode,
            "released_support_method": coarse_interaction_transcript.support.method,
            "target_reconstruction_residual": reconstruction_residual,
            "target_reconstruction_tolerance": float32_tolerance,
            "uncertainty_status": "exact_block_full_covariance",
        }
        log(
            "Loaded released-support coarsened interaction precision: "
            f"mode={coarse_interaction_transcript.mode}, "
            f"rank={optimization_precision.diagnostics()['effective_rank']}, "
            f"dimension={optimization_precision.dimension}"
        )
    ...
    rce_enabled = bool(rce_cfg.get("enabled", False))
    rce_controller: (
        RCEQDTEController
    ...
    initial_rce_regularizer = 0.0
    initial_rce_lagrangian = initial_optimization_objective
    if rce_enabled:
        is_orthogonal_rce = bool(
            precision_operator_name == "orthogonal_interaction"
            and isinstance(optimization_precision, OrthogonalInteractionPrecision)
        )
        is_coarsened_rce = bool(
            precision_operator_name == "coarsened_interaction_full_covariance"
            and isinstance(optimization_precision, CoarsenedInteractionPrecision)
        )
        if not (is_orthogonal_rce or is_coarsened_rce):
            raise ValueError(
                "RCE-v1 requires exact orthogonal or coarsened full-covariance precision"
            )
        if entropy_enabled or entropy_state is not None:
            raise ValueError("RCE-v1 and the legacy entropy profile are mutually exclusive")
        if confidence_stop is not None:
            raise ValueError("RCE-v1 uses its own mixed confidence set")
        if fission_enabled or bool(qdte_cfg.get("structured_swap_enabled", False)):
            raise ValueError("RCE-v1 does not compose fission or structured swaps")
        if str(qdte_cfg.get("candidate_backend", "cpu_repair")) != "cpu_repair":
            raise ValueError("RCE-v1 requires the cpu_repair candidate backend")
        if rce_prefix_backend == "feature_gpu" and not any(
            device.platform == "gpu" for device in jax.devices()
        ):
            raise RuntimeError(
                "qdte.rce.prefix_backend='feature_gpu' requires a visible JAX GPU"
            )
        common_rce_arguments = {
            "qcat": qcat,
            "released_target": state.target,
            "cardinalities": schema.cardinalities,
            "public_total": mechanism_num_rows,
            "initial_rows": state.X_syn,
            "initial_answers": state.answer_syn,
            "initial_residual": state.residual,
            "precision": optimization_precision,
            "max_iterations": int(qdte_cfg.get("max_iters", 5000)),
            "alpha_l2": float(rce_cfg.get("alpha_l2", 0.025)),
            "alpha_linf": float(rce_cfg.get("alpha_linf", 0.025)),
            "product_prior_smoothing": float(
                rce_cfg.get("product_prior_smoothing", 1.0)
            ),
        }
        if streamwise_cdwf_enabled:
            if not is_orthogonal_rce or sequential_cdwf_transcript is None:
                raise RuntimeError(
                    "C3 streamwise RCE lost its exact orthogonal sequential transcript"
                )
            if str(rce_cfg.get("reference_prior")) != (
                "released_confidence_forest_v1"
            ):
                raise ValueError("C3 streamwise RCE requires the released CCF prior")
            streamwise_confidence = (
                sequential_cdwf_transcript.streamwise_confidence()
            )
            sequential_prior = build_sequential_confidence_forest_prior(
                qcat,
                state.target,
                schema.cardinalities,
                sequential_cdwf_transcript.ccf_pair_streams(),
                public_total=mechanism_num_rows,
                smoothing=float(rce_cfg.get("product_prior_smoothing", 1.0)),
            )
            rce_controller = StreamwiseRCEQDTEController.create(
                released_target=state.target,
                initial_rows=state.X_syn,
                initial_answers=state.answer_syn,
                initial_residual=state.residual,
                precision=optimization_precision,
                confidence=streamwise_confidence,
                prior=sequential_prior,
                max_iterations=int(qdte_cfg.get("max_iters", 5000)),
            )
        elif is_coarsened_rce:
            if coarse_interaction_transcript is None:
                raise RuntimeError("Coarsened RCE lost its released transcript")
            if str(rce_cfg.get("reference_prior")) != (
                "released_confidence_forest_v1"
            ):
                raise ValueError("Coarsened RCE requires the released CCF prior")
            rce_controller = CoarsenedRCEQDTEController.create(
                **common_rce_arguments,
                interaction_transcript=coarse_interaction_transcript,
            )
        else:
            if interaction_transcript is None:
                raise RuntimeError("Orthogonal RCE lost its released transcript")
            rce_controller = RCEQDTEController.create(
                **common_rce_arguments,
                reference_prior=str(
                    rce_cfg.get("reference_prior", "released_product")
                ),
                interaction_transcript=interaction_transcript,
                support_coarsening=c0_support,
            )
        initial_rce_regularizer = rce_controller.regularizer
        initial_rce_lagrangian = rce_controller.lagrangian_value(state.residual)
        initial_rce_confidence = rce_controller.current_confidence(state.residual)
        log(
            "Enabled row-realizable confidence-set entropic QDTE: "
            f"prior={rce_controller.prior.diagnostics()['method']}, "
            f"prefix_backend={rce_prefix_backend}, "
            f"regularizer={initial_rce_regularizer:.6g}, "
            f"kl_per_row={initial_rce_regularizer:.6g}, "
            f"ellipsoid_ratio={initial_rce_confidence['ellipsoid_ratio']:.6g}, "
            f"tube_ratio={initial_rce_confidence['tube_ratio']:.6g}, "
            f"slack={initial_rce_confidence['slack']:.6g}"
        )
    ...
    rce_prefix_static_capacity = max(
        accepted_per_iter,
    ...
    orthogonal_precision_score_context = (
        prepare_coarsened_precision_score_context(optimization_precision)
        if isinstance(optimization_precision, CoarsenedInteractionPrecision)
        else prepare_orthogonal_precision_score_context(optimization_precision)
        if isinstance(
            optimization_precision,
            (
                OrthogonalInteractionPrecision,
    ...
    for iteration in range(1, effective_max_iters + 1):
        ...
        rce_dual_update: dict[str, float | int] | None = None
        if rce_controller is not None:
            rce_dual_update = rce_controller.update_dual(state.residual)
        ...
        rce_scored_in_precision_backend = False
        scoring_lambda_cost = (
            0.0
            if entropy_state is not None or rce_controller is not None
            else lambda_cost
        )
            ...
            if objective_loss == "tvd_l1":
                ...
                if rce_controller is not None:
                    if not isinstance(
                        ...
                        (
                            OrthogonalInteractionPrecision,
                            CoarsenedInteractionPrecision,
                        ),
                    ):
                        raise RuntimeError("RCE-v1 lost its exact precision operator")
                    advantages, precision_candidate_quadratic = (
                        rce_controller.candidate_scores(
                            candidates,
                            state.residual,
                    ...
                    rce_scored_in_precision_backend = True
                elif isinstance(
                    optimization_precision,
                    (
                        OrthogonalInteractionPrecision,
        ...
        if entropy_state is not None:
        ...
        elif rce_controller is not None and not rce_scored_in_precision_backend:
            if fused_advantages is not None:
                raise RuntimeError("RCE-v1 does not support fused GPU candidate scoring")
            if not isinstance(
                optimization_precision,
                (OrthogonalInteractionPrecision, CoarsenedInteractionPrecision),
            ):
                raise RuntimeError("RCE-v1 lost its exact precision operator")
            t_rce_score = time.perf_counter()
            advantages, precision_candidate_quadratic = (
                rce_controller.candidate_scores(
                    candidates,
                    state.residual,
            ...
            stats.time_scoring_seconds += time.perf_counter() - t_rce_score
        ...
        before_rce_regularizer = (
            rce_controller.regularizer
            if rce_controller is not None
            else 0.0
        )
        before_regularized_objective = (
            ...
            if entropy_controller is not None
                ...
                rce_controller.lagrangian_value(state.residual)
                if rce_controller is not None
        ...
        rce_prefix_evaluator = None
        rce_index_prefix_evaluator = None
        if rce_controller is not None:

            def rce_index_prefix_evaluator(
                selected_indices: np.ndarray,
                cost_prefix: np.ndarray,
            ) -> np.ndarray:
                prefix_indices = np.asarray(selected_indices, dtype=np.int32)
                if rce_prefix_backend == "feature_gpu":
                    if orthogonal_precision_score_context is None:
                        raise RuntimeError(
                            "GPU RCE prefix scoring requires an orthogonal score context"
                        )
                    return rce_controller.prefix_advantages_from_rows_gpu(
                        query_residual=state.residual,
                        old_rows=candidates.old_rows[prefix_indices],
                        new_rows=candidates.new_rows[prefix_indices],
                        edit_cost_prefix=np.asarray(cost_prefix, dtype=np.float64),
                        lambda_cost=lambda_cost,
                        context=orthogonal_precision_score_context,
                        static_capacity=rce_prefix_static_capacity,
                    )
                return rce_controller.prefix_advantages_from_rows(
                    query_residual=state.residual,
                    old_rows=candidates.old_rows[prefix_indices],
                    new_rows=candidates.new_rows[prefix_indices],
                    edit_cost_prefix=np.asarray(cost_prefix, dtype=np.float64),
            ...
            if atom_flow_update_mode == "exact":
                ...
                transport = choose_atom_flow_batch_transport(
                    ...
                    prefix_advantage_evaluator=(
                        rce_prefix_evaluator
                        if rce_prefix_evaluator is not None
                        else entropy_prefix_evaluator
                    ),
                    prefix_index_advantage_evaluator=rce_index_prefix_evaluator,
                )
            else:
        ...
        accepted_rce_regularizer_gain: float | None = None
        if len(transport.accepted_indices) > 0:
            ...
            if rce_controller is not None:
                accepted_old_rows = candidates.old_rows[transport.accepted_indices]
                accepted_new_rows = candidates.new_rows[transport.accepted_indices]
                exact_batch_advantage, accepted_rce_regularizer_gain = (
                    rce_controller.batch_gain(
                        query_residual=state.residual,
                        query_delta_sum=transport.delta_sum,
                        old_rows=accepted_old_rows,
                        new_rows=accepted_new_rows,
                        edit_cost=accepted_cost,
                        lambda_cost=lambda_cost,
                    )
                )
                transport.diagnostics.update(
                    {
                        "rce_regularizer_batch_gain": float(
                            accepted_rce_regularizer_gain
                        ),
                        "rce_ellipsoid_dual": float(
                            rce_controller.dual.ellipsoid_weight
                        ),
                        "rce_active_positive_duals": int(
                            np.sum(rce_controller.dual.tube_positive > 0.0)
                        ),
                        "rce_active_negative_duals": int(
                            np.sum(rce_controller.dual.tube_negative > 0.0)
                        ),
                    }
                )
            elif entropy_state is not None:
                if entropy_controller is None or optimization_precision is None:
        ...
        if len(transport.accepted_indices) > 0:
            if rce_controller is not None:
                applied_rce_gain = rce_controller.apply_batch(
                    candidates.old_rows[transport.accepted_indices],
                    candidates.new_rows[transport.accepted_indices],
                )
                if accepted_rce_regularizer_gain is None or not np.isclose(
                    applied_rce_gain,
                    accepted_rce_regularizer_gain,
                    rtol=1.0e-10,
                    atol=1.0e-10,
                    ...
                    raise AssertionError(
                        "Applied RCE KL batch gain differs from its exact certificate"
                    )
            elif entropy_state is not None:
            ...
            if rce_controller is not None:
                rce_controller.consider(
                    rows=state.X_syn,
                    answers=state.answer_syn,
                    query_residual=state.residual,
                    iteration=iteration,
                )
        ...
        if debug_recompute_after_batch and (
            ...
            if rce_controller is not None:
                rce_controller.assert_matches(state.X_syn)
                recomputed_regularized_objective = rce_controller.lagrangian_value(
                    recomputed_residual
                )
            elif entropy_state is not None:
        ...
        cur_rce_confidence = (
            rce_controller.current_confidence(state.residual)
            if rce_controller is not None
            else None
        )
        cur_unweighted_loss = unweighted_measured_loss(state.residual)
        if (
            ...
            row = {
                ...
                "rce_regularizer": cur_rce_regularizer,
                "rce_kl_per_row": (
                    cur_rce_regularizer
                    if rce_controller is not None
                    else 0.0
                ...
                "rce_ellipsoid_ratio": (
                    float(cur_rce_confidence["ellipsoid_ratio"])
                    if cur_rce_confidence is not None
                    else 0.0
                ),
                "rce_tube_ratio": (
                    float(cur_rce_confidence["tube_ratio"])
                    if cur_rce_confidence is not None
                    else 0.0
                ),
                "rce_confidence_slack": (
                    float(cur_rce_confidence["slack"])
                    if cur_rce_confidence is not None
                    else 0.0
                ),
                "rce_inside_confidence_set": int(
                    bool(cur_rce_confidence["inside"])
                    if cur_rce_confidence is not None
                    else False
                ),
                "rce_ellipsoid_dual": (
                    float(rce_controller.dual.ellipsoid_weight)
                    if rce_controller is not None
                    else 0.0
                ),
                "rce_active_positive_duals": (
                    int(np.sum(rce_controller.dual.tube_positive > 0.0))
                    if rce_controller is not None
                    else 0
                ),
                "rce_active_negative_duals": (
                    int(np.sum(rce_controller.dual.tube_negative > 0.0))
                    if rce_controller is not None
                    else 0
                ),
                "rce_dual_ellipsoid_constraint": (
                    float(rce_dual_update["ellipsoid_constraint"])
                    if rce_dual_update is not None
                    else 0.0
                ),
            ...
            log(
                ...
                f"rce_kl={cur_rce_regularizer:.6g} "
                f"rce_slack={float(cur_rce_confidence['slack']) if cur_rce_confidence is not None else 0.0:.6g} "
    ...
    rce_selected_iteration: int | None = None
    if rce_controller is not None:
        rce_snapshot = rce_controller.restore_incumbent()
        rce_selected_iteration = int(rce_snapshot.iteration)
        state.X_syn = rce_snapshot.rows.copy()
        state.answer_syn = rce_snapshot.answers.copy()
        state.residual = (state.target - state.answer_syn).astype(np.float32)
        snapshot_confidence = rce_snapshot.confidence
        if isinstance(snapshot_confidence, dict):
            snapshot_ellipsoid_ratio = float(snapshot_confidence["ellipsoid_ratio"])
            snapshot_tube_ratio = float(snapshot_confidence["tube_ratio"])
            snapshot_slack = float(snapshot_confidence["slack"])
        else:
            snapshot_ellipsoid_ratio = float(snapshot_confidence.ellipsoid_ratio)
            snapshot_tube_ratio = float(snapshot_confidence.tube_ratio)
            snapshot_slack = float(snapshot_confidence.slack)
        log(
            "RCE integer incumbent selection: "
            f"selected_iter={rce_selected_iteration}, "
            f"terminal_iter={stats.num_iterations}, "
            f"regularizer={rce_snapshot.regularizer:.6g}, "
            f"ellipsoid_ratio={snapshot_ellipsoid_ratio:.6g}, "
            f"tube_ratio={snapshot_tube_ratio:.6g}, "
            f"slack={snapshot_slack:.6g}"
        )
    ...
    if rce_controller is not None:
        rce_controller.assert_matches(state.X_syn)
        final_rce_regularizer = rce_controller.regularizer
        final_rce_lagrangian = rce_controller.lagrangian_value(state.residual)
        rce_diagnostics = rce_controller.diagnostics(state.residual)
    else:
        final_rce_regularizer = 0.0
        final_rce_lagrangian = final_optimization_objective
        rce_diagnostics = {
            "enabled": False,
            "method": "none",
            "confidence_set": None,
    ...
    if rce_controller is not None:
        final_rce_confidence = rce_diagnostics["final_confidence"]
        log(
            "Final RCE state: "
            f"regularizer={final_rce_regularizer:.6g}, "
            f"kl_per_row={final_rce_regularizer:.6g}, "
            f"lagrangian={final_rce_lagrangian:.6g}, "
            f"ellipsoid_ratio={final_rce_confidence['ellipsoid_ratio']:.6g}, "
            f"tube_ratio={final_rce_confidence['tube_ratio']:.6g}, "
            f"slack={final_rce_confidence['slack']:.6g}"
        )
    ...
    final_metrics: dict[str, Any] = {
        ...
        "rce_enabled": bool(rce_controller is not None),
        "rce_prefix_backend": rce_prefix_backend,
        "rce_diagnostics": rce_diagnostics,
        "offline_c0_diagnostic": {
            "enabled": c0_enabled,
            "protocol_id": C0_PROTOCOL if c0_enabled else None,
            "support_mode": c0_cfg.get("support_mode") if c0_enabled else None,
            "interaction_center": (
                c0_cfg.get("interaction_center") if c0_enabled else None
            ),
            "private_truth_used_by_mechanism": bool(
                c0_enabled
                and (
                    c0_cfg.get("support_mode") == "oracle"
                    or c0_cfg.get("interaction_center") == "clean"
                )
            ),
            "promotion_eligible": False if c0_enabled else None,
        },
        "offline_precision_homotopy": {
            "enabled": precision_homotopy_enabled,
            "protocol_id": (
                PRECISION_HOMOTOPY_PROTOCOL
            ...
            "gamma": (
                precision_homotopy_cfg.get("gamma")
                if precision_homotopy_enabled
                else None
            ),
            "private_truth_used_by_mechanism": precision_homotopy_enabled,
            "promotion_eligible": False if precision_homotopy_enabled else None,
        },
        "rce_selected_iteration": rce_selected_iteration,
        "initial_rce_regularizer": float(initial_rce_regularizer),
        "final_rce_regularizer": float(final_rce_regularizer),
        "rce_regularizer_reduction": float(
            initial_rce_regularizer - final_rce_regularizer
        ),
        "initial_rce_kl_per_row": float(initial_rce_regularizer),
        "final_rce_kl_per_row": float(final_rce_regularizer),
        "rce_regularizer_definition": "D_KL(p_empirical||p0)",
        "rce_effective_lambda_cost": float(
            rce_controller.effective_lambda_cost(lambda_cost)
            if rce_controller is not None
            else 0.0
        ),
        "initial_rce_lagrangian": float(initial_rce_lagrangian),
        "final_rce_lagrangian": float(final_rce_lagrangian),
        ...
        "proposal_precision": (
            ...
            if (
                precision_operator_name.startswith("orthogonal_interaction")
                or precision_operator_name
        ...
        "rce_prefix_backend": rce_prefix_backend,
        "rce_prefix_static_capacity": int(rce_prefix_static_capacity),
    ...
    runtime_dict["rce_enabled"] = bool(rce_controller is not None)
    runtime_dict["rce_prefix_backend"] = rce_prefix_backend
    runtime_dict["rce_prefix_static_capacity"] = int(rce_prefix_static_capacity)
    runtime_dict["rce_selected_iteration"] = rce_selected_iteration
    runtime_dict["initial_rce_regularizer"] = float(initial_rce_regularizer)
    runtime_dict["final_rce_regularizer"] = float(final_rce_regularizer)
    runtime_dict["initial_rce_kl_per_row"] = float(initial_rce_regularizer)
    runtime_dict["final_rce_kl_per_row"] = float(final_rce_regularizer)
    runtime_dict["rce_regularizer_definition"] = "D_KL(p_empirical||p0)"
    runtime_dict["rce_effective_lambda_cost"] = float(
        rce_controller.effective_lambda_cost(lambda_cost)
        if rce_controller is not None
        else 0.0
    )
    runtime_dict["initial_rce_lagrangian"] = float(initial_rce_lagrangian)
    runtime_dict["final_rce_lagrangian"] = float(final_rce_lagrangian)
    runtime_dict["rce_diagnostics"] = rce_diagnostics
    runtime_dict["offline_c0_diagnostic"] = final_metrics[
        "offline_c0_diagnostic"
    ]
    runtime_dict["offline_precision_homotopy"] = final_metrics[
        "offline_precision_homotopy"
    ]
