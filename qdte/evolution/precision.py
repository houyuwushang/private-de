...
class OrthogonalInteractionPrecision(PrecisionOperator):
    ...
    def __init__(
        ...
        for group in workload_groups:
            ...
            if not np.isfinite(variance) or variance < 0.0:
                raise ValueError(
                    f"Transcript block {name!r} must have nonnegative finite variance"
                )
        ...
        if not np.any(self._coefficient_variances > 0.0):
            raise ValueError("Orthogonal interaction precision requires positive stochastic rank")
        self._coefficient_variances.setflags(write=False)
        coefficient_precisions = np.zeros_like(self._coefficient_variances)
        positive = self._coefficient_variances > 0.0
        coefficient_precisions[positive] = 1.0 / self._coefficient_variances[positive]
        coefficient_precisions.setflags(write=False)
        self._coefficient_precisions = coefficient_precisions
    ...
    def weighted_coefficient_residual(self, residual: np.ndarray) -> np.ndarray:
        return self.coefficient_coordinates(residual) * self._coefficient_precisions
    ...
    def prefix_linear_and_diagonal_quadratic(
        self,
        old_rows: np.ndarray,
        new_rows: np.ndarray,
        ...
        """Evaluate cumulative edit prefixes without a dense feature matrix."""

        old = np.asarray(old_rows, dtype=np.int32)
        new = np.asarray(new_rows, dtype=np.int32)
        linear_coefficients = np.asarray(linear_weights, dtype=np.float64)
        quadratic_coefficients = np.asarray(quadratic_weights, dtype=np.float64)
        if old.shape != new.shape or old.ndim != 2:
            raise ValueError("old_rows and new_rows must be matching row matrices")
        if old.shape[1] != len(self._cardinalities):
            raise ValueError(
                f"rows must have shape (n, {len(self._cardinalities)})"
            )
        if linear_coefficients.shape != (self.coefficient_dimension,):
            raise ValueError("linear_weights must match the coefficient dimension")
        if quadratic_coefficients.shape != (self.coefficient_dimension,):
            raise ValueError("quadratic_weights must match the coefficient dimension")
        if not np.all(np.isfinite(linear_coefficients)):
            raise ValueError("linear_weights must be finite")
        if (
            not np.all(np.isfinite(quadratic_coefficients))
            or np.any(quadratic_coefficients < 0.0)
        ):
            raise ValueError("quadratic_weights must be finite and nonnegative")
        for attr, cardinality in enumerate(self._cardinalities):
            if (
                np.any(old[:, attr] < 0)
                or np.any(old[:, attr] >= cardinality)
                or np.any(new[:, attr] < 0)
                or np.any(new[:, attr] >= cardinality)
            ):
                raise ValueError(f"rows contain out-of-range values for attribute {attr}")

        linear = np.zeros(len(old), dtype=np.float64)
        quadratic = np.zeros(len(old), dtype=np.float64)
        changed = old != new
        offset = 0
        for block in self._blocks:
            relevant_mask = changed[:, block.scope[0]].copy()
            if block.kind != "oneway_contrast":
                relevant_mask |= changed[:, block.scope[1]]
            relevant = np.flatnonzero(relevant_mask)
            end = offset + block.rank
            if len(relevant) == 0:
                offset = end
                continue
            left_old = block.left_contrast[old[relevant, block.scope[0]]]
            left_new = block.left_contrast[new[relevant, block.scope[0]]]
            if block.kind == "oneway_contrast":
                ...
                if right is None:
                    raise RuntimeError(
                        "Pair precision block is missing its right contrast"
                    )
                right_old = right[old[relevant, block.scope[1]]]
                right_new = right[new[relevant, block.scope[1]]]
                old_feature = np.einsum(
                    "ni,nj->nij",
                    left_old,
                    right_old,
                    optimize=True,
                ).reshape(len(relevant), -1)
                new_feature = np.einsum(
                    "ni,nj->nij",
                    left_new,
                    right_new,
                    optimize=True,
            ...
            prefix_delta = np.cumsum(edit_delta, axis=0, dtype=np.float64)
            block_linear = linear_coefficients[offset:end]
            block_quadratic = quadratic_coefficients[offset:end]
            linear_increments = np.zeros(len(old), dtype=np.float64)
            linear_increments[relevant] = edit_delta @ block_linear
            linear += np.cumsum(linear_increments, dtype=np.float64)
            quadratic_stages = np.einsum(
                "ij,j,ij->i",
                prefix_delta,
                block_quadratic,
                prefix_delta,
            ...
            quadratic_increments = np.zeros(len(old), dtype=np.float64)
            quadratic_increments[relevant[0]] = quadratic_stages[0]
            if len(relevant) > 1:
                quadratic_increments[relevant[1:]] = np.diff(quadratic_stages)
            quadratic += np.cumsum(quadratic_increments, dtype=np.float64)
            offset = end
        return linear, np.maximum(quadratic, 0.0)
    ...
    def feature_advantages(
        ...
        quad = (deltas * deltas) @ self._coefficient_precisions
    ...
    def feature_quadratic_many(self, feature_deltas: np.ndarray) -> np.ndarray:
        ...
        if deltas.ndim != 2 or deltas.shape[1] != self.coefficient_dimension:
            raise ValueError("feature_deltas have an invalid shape")
        return (deltas * deltas) @ self._coefficient_precisions
    ...
    def matvec(self, values: np.ndarray) -> np.ndarray:
        ...
        for block in self._blocks:
            ...
            if block.variance > 0.0:
                output[block.query_indices] = transformed.reshape(-1) / block.variance
        return output

    def diagnostics(self) -> dict[str, Any]:
        rank = int(sum(block.rank for block in self._blocks if block.variance > 0.0))
        exact_rank = int(sum(block.rank for block in self._blocks if block.variance == 0.0))
        ...
        return {
            ...
            "effective_rank": rank,
            "exact_equality_rank": exact_rank,
            "nullity": int(self.dimension - rank - exact_rank),
            "min_component_variance": float(np.min(positive_variances)),
            "max_component_variance": float(np.max(variances)),
            "num_exact_equality_blocks": int(np.sum(variances == 0.0)),
        }
