...
@jax.jit
def _choose_delta_prefix_fixed_jit(
    deltas: jax.Array,
    edit_cost: jax.Array,
...
def _fixed_atom_flow_pool_size(max_accept: int, pool_multiplier: int, max_pool: int, observed_size: int) -> int:
    ...
    if pool_multiplier > 0:
        return max(int(observed_size), max(int(max_accept), int(pool_multiplier) * int(max_accept)))
    return int(observed_size)


def _pad_indices_for_static_shape(indices: np.ndarray, target_size: int) -> np.ndarray:
    indices = np.asarray(indices, dtype=np.int32)
    if len(indices) >= int(target_size):
        return indices
    if len(indices) == 0:
        return np.zeros(int(target_size), dtype=np.int32)
    return np.pad(indices, (0, int(target_size) - len(indices)), constant_values=int(indices[0])).astype(
        np.int32,
        copy=False,
    )
...
def _select_batch_atom_flow_units(
    ...
    pool_indices = np.asarray(pool_indices, dtype=np.int32)
    pool_advantages = np.asarray(pool_advantages, dtype=np.float32)
    pool_quad = np.asarray(pool_quad, dtype=np.float32)
    old_rows = np.asarray(candidates.old_rows[pool_indices], dtype=np.int32)
    new_rows = np.asarray(candidates.new_rows[pool_indices], dtype=np.int32)
    row_ids = np.asarray(candidates.row_ids[pool_indices], dtype=np.int32)
    local_indices = np.arange(len(pool_indices), dtype=np.int32)

    valid = (
        np.isfinite(pool_advantages)
        & (pool_advantages > float(min_advantage))
        & np.any(old_rows != new_rows, axis=1)
    )
    if not np.any(valid):
        return np.empty(0, dtype=np.int32), np.empty(0, dtype=np.int32), 0, 0, 0

    old_valid = old_rows[valid]
    new_valid = new_rows[valid]
    candidate_valid = pool_indices[valid]
    local_valid = local_indices[valid]
    row_valid = row_ids[valid]
    adv_valid = pool_advantages[valid]
    quad_valid = pool_quad[valid]
    edge_key = np.concatenate([old_valid, new_valid], axis=1)
    _, group_ids = np.unique(edge_key, axis=0, return_inverse=True)
    grouped_edges = int(np.max(group_ids)) + 1
    source_count = int(len(np.unique(old_valid, axis=0)))
    target_count = int(len(np.unique(new_valid, axis=0)))

    # Keep the best candidate per (atom-flow edge, synthetic row), matching the
    # previous dictionary implementation while avoiding per-candidate tuple keys.
    row_order = np.lexsort((local_valid, -adv_valid, row_valid, group_ids))
    group_sorted = group_ids[row_order]
    row_sorted = row_valid[row_order]
    keep_best_row = np.ones(len(row_order), dtype=bool)
    keep_best_row[1:] = (group_sorted[1:] != group_sorted[:-1]) | (row_sorted[1:] != row_sorted[:-1])
    best_order = row_order[keep_best_row]

    best_group = group_ids[best_order]
    best_adv = adv_valid[best_order]
    best_local = local_valid[best_order]
    best_candidate = candidate_valid[best_order]
    best_row = row_valid[best_order]
    best_quad = quad_valid[best_order]

    edge_order = np.lexsort((best_local, -best_adv, best_group))
    sorted_group = best_group[edge_order]
    sorted_adv = best_adv[edge_order]
    sorted_local = best_local[edge_order]
    sorted_candidate = best_candidate[edge_order]
    sorted_row = best_row[edge_order]
    sorted_quad = best_quad[edge_order]

    group_start = np.empty(len(edge_order), dtype=bool)
    group_start[0] = True
    group_start[1:] = sorted_group[1:] != sorted_group[:-1]
    group_start_idx = np.maximum.accumulate(np.where(group_start, np.arange(len(edge_order)), 0))
    flow_rank = np.arange(len(edge_order), dtype=np.int32) - group_start_idx
    ...
    unit_neg_marginal = -marginal[unit_mask]
    unit_local = sorted_local[unit_mask]
    unit_candidate = sorted_candidate[unit_mask]
    unit_row = sorted_row[unit_mask]
    unit_order = np.lexsort((unit_row, unit_candidate, unit_local, unit_neg_marginal))

    used_rows: set[int] = set()
    selected_local: list[int] = []
    selected_indices: list[int] = []
    for order_idx in unit_order.tolist():
        local_idx = int(unit_local[order_idx])
        candidate_idx = int(unit_candidate[order_idx])
        row_id = int(unit_row[order_idx])
        if row_id in used_rows:
            continue
        used_rows.add(row_id)
...
def choose_atom_flow_batch_transport(
    ...
    empty_diagnostics: dict[str, float | int] = {
        ...
        "atom_flow_custom_prefix": int(prefix_advantage_evaluator is not None),
    }
    empty = TransportResult(
        accepted_indices=np.empty(0, dtype=np.int32),
        delta_sum=np.zeros_like(residual, dtype=np.float32),
    ...
    fixed_pool_size = _fixed_atom_flow_pool_size(
        max_accept=max_accept,
        pool_multiplier=pool_multiplier,
        max_pool=max_pool,
        observed_size=len(pool_indices),
    )
    padded_pool_indices = _pad_indices_for_static_shape(pool_indices, fixed_pool_size)
    diagnostics: dict[str, float | int] = {
        "atom_flow_pool_candidates": int(len(pool_indices)),
        "atom_flow_fixed_pool_candidates": int(len(padded_pool_indices)),
        "atom_flow_edges": 0,
        "atom_flow_source_atoms": 0,
        "atom_flow_target_atoms": 0,
        "atom_flow_augments": 0,
    ...
    if orthogonal_feature_mode:
        ...
        deltas_jax = jnp.asarray(
            delta_index.dense_candidate_deltas(
                candidates.old_rows[padded_pool_indices],
                candidates.new_rows[padded_pool_indices],
            ),
            dtype=jnp.int8,
        )
        delta_backend = 1
    ...
    selected_indices, selected_local, grouped_edges, source_count, target_count = _select_batch_atom_flow_units(
        ...
        allow_zero_quadratic=prefix_advantage_evaluator is not None,
    )
    ...
    prefix_capacity = max(1, int(max_accept))
    padded_selected_local = _pad_indices_for_static_shape(selected_local, prefix_capacity)
    ...
    if prefix_advantage_evaluator is not None:
