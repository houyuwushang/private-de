...
def _validate_privacy_ledger(measurements: Measurements) -> dict[str, Any]:
    ...
    ledger = measurements.privacy_ledger
...
def _load_payloads(root: Path) -> tuple[TableSchema, QueryCatalogue, Measurements, dict[str, Any]]:
    ...
    ledger = _validate_privacy_ledger(measurements)
    return schema, queries, measurements, ledger


def write_public_transcript_manifest(root: str | Path) -> dict[str, Any]:
    artifact_root = Path(root).resolve()
    schema, queries, measurements, ledger = _load_payloads(artifact_root)
    artifacts = {
