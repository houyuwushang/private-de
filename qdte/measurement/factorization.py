    ...
    def from_public_dict(cls, data: dict[str, Any]) -> HierarchicalInteractionTranscript:
        ...
        for block in strategy.blocks:
            ...
            if not np.isfinite(variance) or variance <= 0.0:
                raise ValueError(
                    f"Interaction transcript coefficient variance must be positive for {block.name!r}"
                )
            expected_variance = block.sensitivity_l2**2 / (2.0 * rho)
            if not np.isclose(variance, expected_variance, rtol=1.0e-10, atol=1.0e-12):
                raise ValueError(f"Interaction transcript variance/rho mismatch for {block.name!r}")
