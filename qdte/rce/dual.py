...
class RCEDualState:
    ...
    def normalized_constraints(
        ...
        stochastic = self.confidence.stochastic_tube_mask
        bounds = self.confidence.coordinate_bounds[stochastic]
        positive[stochastic] = vector[stochastic] / bounds - 1.0
        negative[stochastic] = -vector[stochastic] / bounds - 1.0
        exact = self.confidence.exact_mask
        scales = self.confidence.dual_coordinate_scales[exact]
        positive[exact] = vector[exact] / scales
        negative[exact] = -vector[exact] / scales
    ...
    def signed_tube_weights(self) -> np.ndarray:
        ...
        ) / self.confidence.dual_coordinate_scales[mask]
        return weights
