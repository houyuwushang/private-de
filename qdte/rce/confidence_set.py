...
class RCEConstraintEvaluation:
    ...
    max_exact_abs_residual: float = 0.0
    num_exact_coordinates: int = 0
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "max_exact_abs_residual": self.max_exact_abs_residual,
            "num_exact_coordinates": self.num_exact_coordinates,
        }
...
class RCEConfidenceSet:
    ...
    def __init__(
        ...
        *,
        ...
        kind: str,
        enforce_zero_variance_equalities: bool = False,
        exact_dual_scale: float = 1.0,
    ) -> None:
        ...
        exact_scale = float(exact_dual_scale)
        if not math.isfinite(exact_scale) or exact_scale <= 0.0:
            raise ValueError("exact_dual_scale must be finite and positive")
        ...
        stochastic_mask = variances > tolerance
        exact_mask = (
            ~stochastic_mask
            if bool(enforce_zero_variance_equalities)
            else np.zeros(variances.shape, dtype=bool)
        )
        tube_mask = stochastic_mask | exact_mask
        tube_dimension = int(np.sum(stochastic_mask))
        if tube_dimension <= 0:
        ...
        stochastic_mask.setflags(write=False)
        exact_mask.setflags(write=False)
        tube_mask.setflags(write=False)
        bounds = np.full(variances.shape, np.inf, dtype=np.float64)
        bounds[stochastic_mask] = c_inf * np.sqrt(variances[stochastic_mask])
        bounds[exact_mask] = 0.0
        bounds.setflags(write=False)
        dual_scales = np.ones(variances.shape, dtype=np.float64)
        dual_scales[stochastic_mask] = bounds[stochastic_mask]
        dual_scales[exact_mask] = exact_scale
        dual_scales.setflags(write=False)

        self._variances = variances
        self._precision_diagonal = diagonal
        self._precision_matrix = matrix
        self._stochastic_mask = stochastic_mask
        self._exact_mask = exact_mask
        self._tube_mask = tube_mask
        self._bounds = bounds
        self._dual_scales = dual_scales
        self._rank = rank
        ...
        self._kind = str(kind)
        self._exact_dual_scale = exact_scale
    ...
    def from_diagonal_variances(
        ...
        *,
        ...
        return cls(
            ...
            kind="diagonal",
            enforce_zero_variance_equalities=enforce_zero_variance_equalities,
            exact_dual_scale=exact_dual_scale,
        )
    ...
    def from_covariance(
        ...
        *,
        ...
        return cls(
            ...
            enforce_zero_variance_equalities=False,
        )
    ...
    @property
    def stochastic_tube_mask(self) -> np.ndarray:
        return self._stochastic_mask

    @property
    def exact_mask(self) -> np.ndarray:
        return self._exact_mask
    ...
    @property
    def dual_coordinate_scales(self) -> np.ndarray:
        return self._dual_scales

    @property
    def precision_diagonal(self) -> np.ndarray | None:
        """Return the immutable diagonal representation when one is available."""

        return self._precision_diagonal
    ...
    def standardized_coordinates(self, residual: np.ndarray) -> np.ndarray:
        ...
        result[self._stochastic_mask] = (
            vector[self._stochastic_mask]
            / np.sqrt(self._variances[self._stochastic_mask])
        )
        return result
    ...
    def evaluate(self, residual: np.ndarray, *, tolerance: float = 1.0e-12) -> RCEConstraintEvaluation:
        ...
        standardized = np.abs(
            self.standardized_coordinates(vector)[self._stochastic_mask]
        )
        maximum = float(np.max(standardized))
        exact_maximum = (
            float(np.max(np.abs(vector[self._exact_mask])))
            if np.any(self._exact_mask)
            else 0.0
        )
        ...
        active = int(
            np.sum(np.abs(standardized / self._c_inf - 1.0) <= 1.0e-6)
            + np.sum(np.abs(vector[self._exact_mask]) <= scale_tolerance)
        )
        return RCEConstraintEvaluation(
            ...
            max_exact_abs_residual=exact_maximum,
            num_exact_coordinates=int(np.sum(self._exact_mask)),
        )
    ...
    def diagnostics(self) -> dict[str, Any]:
        positive = self._variances[self._stochastic_mask]
        return {
            "kind": self._kind,
            "dimension": self.dimension,
            "effective_rank": self.effective_rank,
            "tube_dimension": self.tube_dimension,
            "exact_equality_dimension": int(np.sum(self._exact_mask)),
            "nullity": (
                self.dimension
                - self.effective_rank
                - int(np.sum(self._exact_mask))
            ),
            ...
            "exact_dual_scale": self._exact_dual_scale,
        }
